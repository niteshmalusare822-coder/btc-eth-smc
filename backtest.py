...
def _open_trade(symbol, df, sig_i, side, level, stop_level, atr, cfg, tf_min,
    ...
    # COST GATE — the backtest must refuse what live refuses.
    #
    # app.py has always computed tradeable = cost_in_r <= MAX_COST_IN_R and
    # shown it on the card, but the backtest only RECORDED cost_in_r and took
    # the trade anyway. So every result was averaged over trades the live
    # system would have flagged as untradeable.
    #
    # cost_in_r = round_trip_cost% / stop_distance%. Measured on real setups:
...
def run_arm(symbol, df5, ctx, arm, cfg, lo_i, hi_i, rng=None, matched=None):
    ...
    tf_min = 5
    trades, rejected = [], {}
    # ONE ENTRY PER SETUP. active_setups_at() returns the same live zone on
    # every 5M bar until it expires, so decide() answers OK repeatedly for one
    # setup at one entry level. SMC enters an order block once, on the retest.
    traded_setups = set()
    busy_until = -1
    ...
    for i in range(lo_i, hi_i):
        ...
        setup_key = (s.confirmed_ts, s.entry_level, s.side)
        ...
        if tr:
            ...
            if mode == "A":
                ...
                # WAS: mode == "c". Lowercase, so it never matched. The
                # blocking test above reads open_until, which therefore stayed
                # at -1 forever and blocked nothing. Under the default
                # position_mode "B" the engine opened a fresh position on
                # every bar while one was already running: 3043 SMC trades on
                # ETH where 87 distinct opportunities existed. Those were not
                # independent observations, so no verdict built on them meant
                # anything.
