...
CONFIG = {
    ...
    'SCORE_THRESHOLD': 3,           # ✅ 5 → 3 (zyada signals milenge)
    'ATR_COMPRESSION_RATIO': 0.7,
    'ATR_MA_PERIOD': 50,
    'CHOPPINESS_PERIOD': 14,
    'CHOPPINESS_TREND_MAX': 61.8,
    'LIMIT': 200,
    'TP_ATR_MULT': 3.0,             # TP = entry ± 3x ATR
    'SL_ATR_MULT': 1.5,             # SL = entry ∓ 1.5x ATR
    'RSI_OVERBOUGHT': 80,           # ✅ BUY block above this
    'RSI_OVERSOLD': 20,             # ✅ SELL block below this
}
...
def analyze_timeframe(df):
    ...
    return {
        ...
        "ema20": last["ema20"],
        "rsi": last["rsi"],
        "atr": last["atr"],          # ✅ ATR added for TP/SL calc
        "pattern": last["pat_sig"],
...
def get_ltf_scores(snap_5m, snap_15m):
    ...
    for snap, w in [(snap_5m, 1.0), (snap_15m, 1.2)]:
        ...
        if not pd.isna(snap["rsi"]):
            if snap["rsi"] < 30:
                buy_score  += 1.5 * w   # oversold → possible bounce
            elif snap["rsi"] > 70:
                sell_score += 1.5 * w   # overbought → possible reversal

    return round(buy_score, 2), round(sell_score, 2)


def decide_direction(buy_score, sell_score, htf_bias, entry_adx,
                     regime_5m, regime_15m, entry_rsi=None):
    # ADX filter
    ...
    # ✅ RSI extreme filter — don't enter overbought BUY or oversold SELL
    if entry_rsi is not None and not pd.isna(entry_rsi):
        if entry_rsi > CONFIG['RSI_OVERBOUGHT']:
            return None, f"BLOCKED (RSI overbought {entry_rsi:.1f} > {CONFIG['RSI_OVERBOUGHT']})"
        if entry_rsi < CONFIG['RSI_OVERSOLD']:
            return None, f"BLOCKED (RSI oversold {entry_rsi:.1f} < {CONFIG['RSI_OVERSOLD']})"
    # Signal logic
...
def calc_tp_sl(direction, price, atr):
    """
    2:1 Risk Reward
    LONG:  SL = price - 1.5*ATR  |  TP = price + 3*ATR
    SHORT: SL = price + 1.5*ATR  |  TP = price - 3*ATR
    """
...
def _get_ltf_snaps_cached(symbol):
    ...
    if cached and (now - cached["ts"]) < _LTF_CACHE_TTL:
        return cached["snap_5m"], cached["snap_15m"]
    ...
    snap_15m = analyze_timeframe(df_15m) if df_15m is not None else None
    _LTF_CACHE[symbol] = {"snap_5m": snap_5m, "snap_15m": snap_15m, "ts": now}
    return snap_5m, snap_15m
...
def analyze(symbol, timeframe="5m"):
    ...
    snap_5m, snap_15m = _get_ltf_snaps_cached(symbol)
    if snap_5m  is None: snap_5m  = snap_entry
    if snap_15m is None: snap_15m = snap_entry

    buy_score, sell_score = get_ltf_scores(snap_5m, snap_15m)

    direction, reason = decide_direction(
        buy_score, sell_score, htf_bias, snap_entry["adx"],
        snap_5m["regime"], snap_15m["regime"],
        entry_rsi=rsi_now          # ✅ RSI filter pass
    )
    ...
    return {
        ...
        "exchange":   ex_id,
        "entry":      round(price, 4) if direction else None,   # ✅ Entry
        "tp":         tp,                                        # ✅ Take Profit
        "sl":         sl,                                        # ✅ Stop Loss
        "atr":        round(atr_now, 4) if atr_now else None,   # ✅ ATR info
    }
