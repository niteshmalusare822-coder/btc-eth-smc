...
# ── HTF bias cache (1h + 4h) — refreshes every 60s ─────
_HTF_CACHE = {}
...
# ── LTF snap cache (5m + 15m) — refreshes every 30s ────
# FIX: Ye cache ensure karta hai ki scoring hamesha
#      proper 5m vs 15m comparison se ho, same snap twice nahi.
_LTF_CACHE = {}
_LTF_CACHE_TTL = 30  # seconds
...
def analyze(symbol, timeframe="5m"):
    ...
    HTF (1h/4h) bias cached 60s | LTF (5m/15m) snaps cached 30s.
    """
    ...
    # HTF bias (1h + 4h) — cached
    htf_bias = _get_htf_bias_cached(symbol)

    # ✅ FIX 1: Proper 5m + 15m snaps — same snap twice nahi!
    ...
    # ✅ FIX 2: Alag-alag snaps pass ho rahe hain scoring mein
    buy_score, sell_score = get_ltf_scores(snap_5m, snap_15m)

    # ✅ FIX 3: Alag-alag regimes pass ho rahe hain direction logic mein
