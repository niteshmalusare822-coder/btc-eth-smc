...
CONFIG = {
    'EMA_FAST': 9,
    'EMA_SLOW': 21,
    'RSI_PERIOD': 14,              # ✅ 7→14 (standard, CoinDCX se match)
    'ATR_PERIOD': 14,
    'ADX_PERIOD': 14,
    'ADX_MIN': 12,                 # ✅ 15→12 (thoda loose, zyada signals)
    'SWING_LOOKBACK': 3,
    'LIQUIDITY_SWEEP_LOOKBACK': 20,
    'VOLUME_PROFILE_LOOKBACK': 100,
    'VOLUME_PROFILE_BINS': 24,
    'SCORE_THRESHOLD': 2.5,        # ✅ 3→2.5 (aur loose)
...
def decide_direction(buy_score, sell_score, htf_bias,
    ...
    # 3. ✅ Compression — only block if 15m is NOT trending
    #    Agar 15m TRENDING hai toh compression ignore karo
    if regime_5m["regime"] == "COMPRESSION" and regime_15m["regime"] != "TRENDING":
        return None, f"BLOCKED (compression, wait for breakout)"

    # 4. 15m ranging block
    if regime_15m["regime"] == "RANGING":
        return None, f"BLOCKED (15m choppy, CI {regime_15m['choppiness']})"

    # 5. ✅ Signal logic — NEUTRAL bias pe bhi signal allow karo
    ...
    return None, "WAIT (score ya bias aligned nahi)"
